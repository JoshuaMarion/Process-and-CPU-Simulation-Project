            ...
            if x[0] == time:  # Hit something to do
                ...
                if x[2] == 0:  # Arrival
                    ...
                    if current_process is None: # (current_process.get_process_name() == process.get_process_name()):
                        # making everything current,
                        current_process = ready_queue.pop(0)
                        tmp_cpu_burst = current_process.get_cpu_burst_array()[process.get_cpu_burst_index()]
                        if (time < 10_000):
                            print(f'time {time}ms: Process {current_process.get_process_name()} started using the CPU for {tmp_cpu_burst}ms burst [Q{print_ready_queue(ready_queue)}]')
                        current_process.increment_cpu_burst_index()
                        current_process.use_cpu_burst()
                        if (current_process.get_num_cpu_bursts() == 0):
                            thing_to_do.append([time + tmp_cpu_burst, current_process, 5, current_process.get_process_name()])
                        else:
                            thing_to_do.append([time + tmp_cpu_burst, current_process, 2, current_process.get_process_name()])
...
def fcfs_loud(process_array, processes, cpu_bound_processes, seed, lambda_, upper_bound, context_switch_time, cpu_burst_time_estimate, time_slice):
    # This is correct for the first test case, not the others
        ...
        for x in thing_to_do:
            ...
            if x[0] == time:  # Hit something to do
                ...
                if x[2] == 0:  # Arrival
                    ...
                    if current_process is None: # (current_process.get_process_name() == process.get_process_name()):
                        current_process = ready_queue.pop(0)
                        tmp_cpu_burst = current_process.get_cpu_burst_array()[current_process.get_cpu_burst_index()]
                        # if (time < 10_000):
                        print(f'time {time}ms: Process {current_process.get_process_name()} started using the CPU for {tmp_cpu_burst}ms burst [Q{print_ready_queue(ready_queue)}]')
                        current_process.increment_cpu_burst_index()
                        current_process.use_cpu_burst()
